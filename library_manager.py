...
def create_visualizations(state):
    if state['total_books'] > 0:
        fig_read_status = go.Figure(data=[go.pie(labels=['Read','Unread'],
        values=[state['read_books'], state['total_books'] - state ['read_books']],
         hole=.4,
    ...
    if state['genres']:
        genres_df = pd.DataFrame({
            'Genre': list(state['genres'].keys()),
            'Count': list(state['genres'].values())
        })
        fig_genres = px.bar(
            genres_df,
            x='Genre',
    ...
    if state['decades']:
        decades_df = pd.DataFrame({
            'Decade': [f"{decade}s" for decade in state['decades'].keys()],
            'Count': list(state['decades'].values())
        })
...
lottie_book = load_lottieurl("https://assets9.lottiefiles.com/temp.json")
if lottie_book:
    with st.sidebar:
...
if stats['authors']:
    ...
    top_authors = dict(list(stats['authors'].items())[:5])  # Get the top 5 authors
    ...
    # Get library stats
    stats = get_library_state()  # This should match the function that gets library statistics
    ...
    # Create and display visualizations
    create_visualizations(stats)  # Display the visualizations with stats

# Footer
st.markdown("---")
st.markdown("© 2025 Sheerz Ahmed Personal Library Manager | Created with Love and Streamlit", unsafe_allow_html=True)
st.markdown("© 2025 Sheerz Ahmed Personal Library Manager", unsafe_allow_html=True)
